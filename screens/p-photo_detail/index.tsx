...
import { ActivityIndicator, Clipboard, Dimensions, FlatList, Image, Modal, PanResponder, ScrollView, Text, TouchableOpacity, View } from 'react-native';
...
const PhotoDetailScreen = () => {
  ...
  const { photoId = 'default' } = useLocalSearchParams();
  ...
  const [dislikedContent, setDislikedContent] = useState<Record<string, boolean>>({});
  ...
  const handleRegenerateContent = (type: ContentType | 'all') => {
    ...
    setTimeout(() => {
      if (photo) {
        const availableIds = Object.keys(PHOTO_DATA).filter(id => id !== photoId);
  ...
  const handlePhotoSwipe = (direction: 'left' | 'right') => {
    ...
    const availableIds = Object.keys(PHOTO_DATA);
    const currentIndex = availableIds.indexOf(photoId as string);
    
    let newIndex;
    if (direction === 'left') {
      newIndex = (currentIndex + 1) % availableIds.length;
    } else {
      newIndex = (currentIndex - 1 + availableIds.length) % availableIds.length;
    }
    
    const newId = availableIds[newIndex];
    router.push(`/p-photo_detail?photoId=${newId}`);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.bottomActions}>
        ...
        >
          <FontAwesome5 name="sync-alt" size={16} color="#E2E8F0" style={styles.buttonIcon} />
        ...
        >
          <FontAwesome6 name="copy" size={16} color="#FFFFFF" style={styles.buttonIcon} />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.copyOptionsContainer}>
            ...
            <View style={styles.copyOptionsList}>
              ...
              >
                <FontAwesome6 name="quote-left" size={16} color="#38BDF8" style={styles.copyOptionIcon} />
              ...
              >
                <FontAwesome6 name="paragraph" size={16} color="#38BDF8" style={styles.copyOptionIcon} />
              ...
              <TouchableOpacity 
                ...
                onPress={() => handleCopyContent('tags')}
              >
                <FontAwesome6 name="tags" size={16} color="#38BDF8" style={styles.copyOptionIcon} />
              ...
              >
                <FontAwesome5 name="feather-alt" size={16} color="#38BDF8" style={styles.copyOptionIcon} />
              ...
              >
                <FontAwesome6 name="copy" size={16} color="#FFFFFF" style={styles.copyOptionIcon} />
      ...
      {isRegenerating && (
        <View style={styles.regeneratingOverlay}>
          <View style={styles.regeneratingContent}>
            <ActivityIndicator size="large" color="#38BDF8" style={styles.regeneratingSpinner} />
...
const PHOTO_DATA: Record<string, PhotoData> = {
  'default': {
    ...
    content: {
      ...
      promotion: '置身于这壮丽的山脉之间，仿佛能听见大自然的呼吸。蓝天白云下，层峦叠嶂的山脉绵延不绝，每一座山峰都诉说着亘古的故事。这里的空气清新，视野开阔，让人忘却城市的喧嚣，找回内心的宁静。如果你也向往这样的自然美景，不妨放下手中的工作，来一场说走就走的旅行，让心灵在山水间得到真正的放松。',
      tags: ['#自然风光', '#山水之间', '#旅行日记', '#治愈系风景', '#户外探险', '#摄影爱好者'],
  ...
  'photo1': {
    ...
    content: {
      ...
      promotion: '夜幕降临，城市的灯火次第点亮，勾勒出现代都市的轮廓。高楼大厦间，车水马龙的街道上，是无数奔波忙碌的身影。这座不夜城，承载着太多人的梦想与希望，每一盏灯背后都有一个故事。站在高处俯瞰，城市的脉搏清晰可见，这是一幅充满活力与生机的画卷。',
      tags: ['#城市夜景', '#都市生活', '#灯火阑珊', '#现代都市', '#夜色迷人', '#城市摄影'],
  ...
  'photo2': {
    ...
    content: {
      ...
      promotion: '穿行在这片古老的森林中，阳光透过茂密的树冠，洒下斑驳的光影。空气中弥漫着泥土和植物的清新气息，耳边是鸟儿欢快的鸣叫和溪水潺潺的声音。这里远离尘嚣，是大自然赐予我们的净土，每一棵树、每一片叶都在诉说着生命的奇迹。在这片绿色的海洋中，仿佛所有的烦恼都能得到释放，找回最纯粹的自己。',
      tags: ['#森林探险', '#大自然', '#绿色世界', '#户外徒步', '#生态摄影', '#治愈系'],
